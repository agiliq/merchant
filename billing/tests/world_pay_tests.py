...
class WorldPayTestCase(TestCase):
    def setUp(self):
        ...
        signature = md5("%s:%s:%s:%s" %(settings.WORLDPAY_MD5_SECRET_KEY, self.wp.fields["instId"],
                                        self.wp.fields["amount"], self.wp.fields["cartId"])).hexdigest()
        self.wp.add_field("signature", signature)
