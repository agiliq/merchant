...
class OgonePaymentsNode(template.Node):
    def __init__(self, integration):
        self.integration = template.Variable(integration)

    def render(self, context):
        int_obj = self.integration.resolve(context)
        form_str = render_to_string("billing/ogone_payments.html",
                                    {"integration": int_obj},
                                    context)
        return form_str

@register.tag
def ogone_payments(parser, token):
    try:
    ...
    return OgonePaymentsNode(int_obj)
